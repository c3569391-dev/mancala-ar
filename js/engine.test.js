...
(function () {
  ...
  eq(s.board, [4, 4, 4, 4, 4, 4, 4, 0, 4, 4, 4, 4, 4, 4, 4, 0], '1a initial board');
  eq(s.current, 'A', '1b A starts');
  ok(!s.over, '1c not over');
  eq(E.legalMoves(s), [0, 1, 2, 3, 4, 5, 6], '1d A has seven legal starting moves');
  eq(E.legalMoves(S([4,4,4,4,4,4,4,0, 4,4,4,4,4,4,4,0], 'B')), [8, 9, 10, 11, 12, 13, 14], '1e B has seven legal starting moves');
  eq(E.legalMoves(S([0,0,0,0,0,0,0,0, 4,4,4,4,4,4,4,0], 'A')), [], '1f no moves when row empty');
  eq([0,1,2,3,4,5,6].map(E.opposite), [14,13,12,11,10,9,8], '1g opposite mapping uses 14 - i');
})();
...
(function () {
  ...
  eq(r.state.board, [0, 5, 5, 5, 5, 4, 4, 0, 4, 4, 4, 4, 4, 4, 4, 0], '2b board after sow');
...
// =====================================================================
(function () {
  // Player A must SKIP Player B's store (15).
  // A plays pit 6 with 9 seeds -> 7,8,9,10,11,12,13,14, then skip 15 -> 0
  var sa = S([4,4,4,4,4,4,9, 0, 4,4,4,4,4,4,4, 0], 'A');
  var ra = E.applyMove(sa, 6);
  eq(ra.events[0] && ra.events[0].path, [7, 8, 9, 10, 11, 12, 13, 14, 0], '3a A skips B store (15)');
  ok(ra.state.board[15] === 0, '3b B store untouched by A');

  // Player B must SKIP Player A's store (7).
  // B plays pit 14 with 9 seeds -> 15,0,1,2,3,4,5,6, then skip 7 -> 8
  var sb = S([4,4,4,4,4,4,4, 0, 4,4,4,4,4,4,9, 0], 'B');
  var rb = E.applyMove(sb, 14);
  eq(rb.events[0] && rb.events[0].path, [15, 0, 1, 2, 3, 4, 5, 6, 8], '3c B skips A store (7)');
  ok(rb.state.board[7] === 0, '3d A store untouched by B (no seed added, stays 0)');
})();
...
// =====================================================================
(function () {
  // A plays pit 3 (4 seeds) -> 4,5,6,7 ; last in A store -> extra turn
  var r = E.applyMove(E.createInitialState(), 3);
  ...
  eq(r.state.board[7], 1, '4c seed landed in A store');

  // B earns an extra turn too (pit 11 with 4 seeds -> 12,13,14,15)
  var rb = E.applyMove(S([4,4,4,4,4,4,4, 0, 4,4,4,4,4,4,4, 0], 'B'), 11);
...
// =====================================================================
(function () {
  // A plays pit 0 (3) -> 1,2,3 ; pit 3 was empty (own side); opposite 11 has 4
  var s = S([3,0,0,0,4,4,4, 0, 4,4,4,4,4,4,4, 0], 'A');
  var r = E.applyMove(s, 0);
  var cap = r.events.find(function (e) { return e.type === 'capture'; });
  eq(cap, { type: 'capture', player: 'A', pit: 3, opposite: 11, gained: 5 }, '5a capture event');
  eq(r.state.board[7], 5, '5b 5 seeds into A store');
  eq(r.state.board[3], 0, '5c landing pit emptied');
  eq(r.state.board[11], 0, '5d opposite pit emptied');
...
(function () {
  ...
  // A plays pit 0 (1) -> pit 1 (own, empty); opposite 13 is empty -> no capture
  var r1 = E.applyMove(S([1,0,4,4,4,4,4, 0, 4,4,4,4,4,0,4, 0], 'A'), 0);
  ...
  // A plays pit 6 (2) -> 7(store),8 ; pit 8 empty but it is B's side -> no capture
  var r2 = E.applyMove(S([4,4,4,4,4,4,2, 0, 0,4,4,4,4,4,4, 0], 'A'), 6);
  ok(!r2.events.some(function (e) { return e.type === 'capture'; }), '6b no capture on opponent side');
  eq(r2.state.board[8], 1, '6b- seed stays in pit 8');

  // 6c No capture when landing in own NON-empty pit.
  // A plays pit 0 (1) -> pit 1 which already had seeds -> no capture
  var r3 = E.applyMove(S([1,3,4,4,4,4,4, 0, 4,4,4,4,4,4,4, 0], 'A'), 0);
...
(function () {
  ...
  // A plays pit 6 (1) -> store 7 ; A row now all empty -> game over.
  var s = S([0,0,0,0,0,0,1, 20, 2,0,0,0,0,0,3, 10], 'A');
  var r = E.applyMove(s, 6);
  var go = r.events.find(function (e) { return e.type === 'gameOver'; });
  ok(!!go, '7a gameOver event fired');
  eq(r.state.board[7], 21, '7b A store = 21 (20 + landed seed)');
  eq(r.state.board[15], 15, '7c B store swept to 15 (10 + 2 + 3)');
  eq(go && go.swept, { A: 0, B: 5 }, '7d swept counts');
  eq(r.state.winner, 'A', '7e A wins (21 > 15)');
  ok(r.state.over && r.state.current === null, '7f game flagged over');
  // every pit empty after sweep
  ok([0,1,2,3,4,5,6,8,9,10,11,12,13,14].every(function (i) { return r.state.board[i] === 0; }), '7g all pits empty');

  // 7h B empties its own row -> A side swept.
  var sb = S([3,0,0,0,0,0,2, 5, 0,0,0,0,0,0,1, 9], 'B');
  var rb = E.applyMove(sb, 14); // pit14 (1) -> store15 ; B row empty -> over
  ok(rb.events.some(function (e) { return e.type === 'gameOver'; }), '7h B-side end detected');
  eq(rb.state.board[7], 10, '7i A swept (5 + 3 + 2)');
  eq(rb.state.board[15], 10, '7j B store = 10');
...
(function () {
  var s = E.createInitialState();
  var r1 = E.applyMove(s, 8); // opponent pit
  ok(r1.illegal === true && r1.state === s, '8a cannot play opponent pit');
  var r2 = E.applyMove(S([0,4,4,4,4,4,4, 0, 4,4,4,4,4,4,4, 0], 'A'), 0); // empty pit
